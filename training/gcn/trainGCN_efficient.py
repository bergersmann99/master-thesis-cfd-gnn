...
VRAM-optimierte Variante von trainGCN.py mit zwei zusaetzlichen
Optionen:
  ...
  1. Gradient Checkpointing (--gradient-checkpointing):
     ...
     Reduziert den VRAM-Bedarf von O(L * N * H) auf O(N * H),
...
Ohne --gradient-checkpointing und --mixed-precision ist das Verhalten
identisch zu trainGCN.py.
...
Verwendung:
    python trainGCN_efficient.py --data-dir ./graph_dataset
    python trainGCN_efficient.py --data-dir ./graph_dataset \\
        --gradient-checkpointing --mixed-precision
    python trainGCN_efficient.py --s3-download \\
        --s3-prefix graph-dataset_building_focus_25 \\
        --gradient-checkpointing --mixed-precision

Siehe QUELLEN_trainGCN_efficient.md fuer vollstaendige Quellenangaben.
"""
...
def setup_file_logger(output_dir):
    ...
    logger = logging.getLogger("trainGCN_efficient")
...
class GCNProcessor(nn.Module):
    ...
    neu berechnet. Dies reduziert den VRAM-Bedarf von O(L * N * H)
    auf O(N * H) auf Kosten von ~30% laengerem Training.
...
def save_checkpoint(model, optimizer, scheduler, epoch, val_loss,
    ...
    checkpoint = {
        ...
        "architecture": "GCN_Encode-Process-Decode_efficient",
...
def save_last_checkpoint(model, optimizer, scheduler, epoch,
                         ...
                         epoch_time_history, timestamp_history,
                         stats, args, output_dir,
                         filename="last_checkpoint.pt"):
    """
    Speichert den vollstaendigen Trainingszustand fuer Crash-Recovery.

    Im Gegensatz zu save_checkpoint() (das nur bei Verbesserung
    best_model.pt schreibt) wird last_checkpoint.pt jede Epoche
    ueberschrieben und enthaelt ZUSAETZLICH:
      - best_val_loss, best_epoch, epochs_without_improvement
        (Early-Stopping-Zustand)
      - RNG-States (torch, cuda, numpy, python) fuer deterministisches
        Fortsetzen

    Damit kann das Training nach Absturz nahtlos ab der naechsten
    Epoche fortgesetzt werden, ohne Patience-Zaehler oder
    Scheduler-Plateau-Erkennung zu verlieren.

    Geschrieben wird atomar via tmp-Datei + os.replace, damit ein
    Crash mitten im Schreiben kein leeres/halbes File hinterlaesst.
    """
    ...
    checkpoint = {
        ...
        "history": {
            "train_loss": list(train_history),
            "val_loss": list(val_history),
            "learning_rate": list(lr_history),
            "epoch_duration_s": list(epoch_time_history),
            "timestamp": list(timestamp_history),
        },
...
def main():
    ...
    parser.add_argument(
        ...
        help="Gradient Checkpointing aktivieren: Aktivierungen "
             ...
             "neu berechnet. Reduziert VRAM im Processor "
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./output_gcn_efficient",
        help="Ausgabeverzeichnis "
             "(Standard: ./output_gcn_efficient)")
    ...
    header_lines = [
        "",
        "=" * 60,
        "   GCN SURROGAT-MODELL TRAINING (VRAM-optimiert)",
    ...
    # Prioritaet: last_checkpoint.pt (vollst. State) > best_model.pt (Fallback)
    # ------------------------------------------------------------------
    ...
    if os.path.exists(last_ckpt_path):
        resume_checkpoint = torch.load(
            last_ckpt_path, weights_only=False)
        resume_kind = "last"
        log_and_print(logger,
                      "   Vorhandener last_checkpoint.pt gefunden — "
                      "Resume-Modus aktiv (voller Zustand).")
    elif os.path.exists(best_ckpt_path):
        resume_checkpoint = torch.load(
            best_ckpt_path, weights_only=False)
        resume_kind = "best"
        log_and_print(logger,
                      "   Kein last_checkpoint.pt — Fallback auf "
                      "best_model.pt (Patience/RNG werden zurueckgesetzt).")
    ...
    if resume_checkpoint is not None:
        import random as _random_resume
        ...
        if resume_kind == "last":
            ...
            epochs_without_improvement = resume_checkpoint[
                "epochs_without_improvement"]
            ...
            if rng:
                ...
                _random_resume.setstate(rng["python"])
                log_and_print(logger,
                              "   RNG-States wiederhergestellt "
                              "(torch/cuda/numpy/python).")
        else:
            # Fallback: best_model.pt (alt-Format ohne best_epoch etc.)
            best_val_loss = resume_checkpoint["val_loss"]
            best_epoch = resume_checkpoint["epoch"]
            epochs_without_improvement = 0
            log_and_print(logger,
                          "   WARNUNG: Patience-Zaehler und RNG werden "
                          "auf Default zurueckgesetzt.")

        history = resume_checkpoint.get("history") or {}
        if history and resume_kind == "last":
            cut = resume_checkpoint["epoch"]
            train_history = list(history.get("train_loss", []))[:cut]
            val_history = list(history.get("val_loss", []))[:cut]
            lr_history = list(history.get("learning_rate", []))[:cut]
            epoch_time_history = list(
                history.get("epoch_duration_s", []))[:cut]
            timestamp_history = list(history.get("timestamp", []))[:cut]
            log_and_print(logger,
                          f"   History aus Checkpoint geladen: "
                          f"{len(train_history)} Epochen.")
        else:
            ...
            if os.path.exists(history_path):
                with open(history_path) as f:
                    hist = json.load(f)
                cut = resume_checkpoint["epoch"]
                train_history = hist.get("train_loss", [])[:cut]
                val_history = hist.get("val_loss", [])[:cut]
                lr_history = hist.get("learning_rate", [])[:cut]
                epoch_time_history = hist.get(
                    "epoch_duration_s", [])[:cut]
                timestamp_history = hist.get("timestamp", [])[:cut]
                log_and_print(logger,
                              f"   History aus training_history.json "
                              f"geladen: {len(train_history)} Epochen.")
            else:
                log_and_print(logger,
                              "   WARNUNG: Keine History gefunden — "
                              "Verlauf vor Resume-Punkt fehlt.")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # Last-Checkpoint jede Epoche fuer Crash-Recovery
    ...
    # ------------------------------------------------------------------
    # Trainingshistorie speichern (mit erweitertem Logging)
    # ------------------------------------------------------------------
    ...
    history_payload = {
        ...
        "epoch_duration_s": epoch_time_history,
