"""
Überwacht das GATv2 Coarse Training und dokumentiert alle 50 Epochen einen Zwischenstand.
"""
import re
import time
import subprocess
...
LOG_FILE     = Path("/home/tbergermann/Python/GAT/output_gatv2_coarse.log")
DOC_FILE     = Path("/home/tbergermann/Python/logs/GATv2/training_GATv2_coarse.md")
TOTAL_EPOCHS = 800
INTERVAL     = 50
POLL_SEC     = 30
...
def is_training_running():
    r = subprocess.run(["pgrep", "-f", "output_gatv2_coarse"], capture_output=True)
...
def append_table_row(doc_path, entry, lr_reductions_total):
    ...
    if marker in content:
        ...
        print(f"[Monitor GATv2 Coarse] Epoche {entry['epoch']} dokumentiert.")
...
def finalize_results(doc_path, log_text):
    ...
    doc_path.write_text(content)
    print("[Monitor GATv2 Coarse] Ergebnisse eingetragen.")

def main():
    print("[Monitor GATv2 Coarse] Gestartet.")
    last_milestone = 0
    ...
    while True:
        ...
        if not is_training_running():
            print("[Monitor GATv2 Coarse] Training abgeschlossen.")
    ...
    print("[Monitor GATv2 Coarse] Beendet.")
