"""
Überwacht das GCN Coarse Training und dokumentiert alle 50 Epochen einen Zwischenstand.
"""
import re
import time
import subprocess
...
def finalize_results(doc_path, log_text):
    ...
    if test_block_m:
        ...
        if gesamt_m:
            ...
            old_gesamt = "| **GESAMT** | — | — | — | — | — |"
            new_gesamt = f"| **GESAMT** | **{gesamt_m.group(1)}** | **{gesamt_m.group(2)}** | **{gesamt_m.group(3)}** | **{r2_total}** | **{rl2_total}** |"
            content = content.replace(old_gesamt, new_gesamt)
...
def main():
    ...
    while True:
        ...
        if not is_training_running():
            print("[Monitor GCN Coarse] Training abgeschlossen.")
    ...
    print("[Monitor GCN Coarse] Beendet.")
