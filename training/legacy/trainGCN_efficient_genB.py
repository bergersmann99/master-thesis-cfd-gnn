...
Verwendung:
    ...
    python trainGCN_efficient.py --s3-download \\
        --s3-prefix graph-dataset_building_focus \\
        --gradient-checkpointing --mixed-precision
...
def log_and_print(logger, msg):
    """
    Schreibt eine Nachricht in Log-Datei UND auf die Konsole (neue Zeile).
...
def log_only(logger, msg):
    """
    Schreibt eine Nachricht nur in die Log-Datei (nicht auf Konsole).
...
def console_live(msg):
    """
    Schreibt eine ueberschreibbare Statuszeile auf die Konsole.

    Verwendet Carriage Return ohne Newline, sodass die naechste
    Ausgabe die aktuelle Zeile ueberschreibt.
...
def download_from_s3(s3_path, local_path, max_retries=3):
    """
    Laedt eine einzelne Datei von S3 herunter (mit Retry-Logik).

    Parameter
    ---------
    s3_path : str
        S3-Pfad (s3://bucket/key).
    local_path : str
        Lokaler Zielpfad.
    max_retries : int
    ...
    aws_cmd = os.path.join(
        os.path.dirname(sys.executable), "aws")
    if not os.path.exists(aws_cmd):
        aws_cmd = "aws"

    for attempt in range(1, max_retries + 1):
        result = subprocess.run(
            [aws_cmd, "s3", "cp", s3_path, local_path],
    ...
    raise RuntimeError(
        f"S3-Download fehlgeschlagen nach {max_retries} Versuchen: "
        f"{s3_path}\n{result.stderr}"
    )
...
def download_dataset_from_s3(s3_bucket, s3_prefix, local_dir):
    """
    Laedt train.pt, val.pt und test.pt von S3 herunter.
    ...
    s3_bucket : str
        S3-Bucket-Name.
    s3_prefix : str
        Prefix (Ordner) im Bucket.
    local_dir : str
        Lokales Zielverzeichnis.
    """
    os.makedirs(local_dir, exist_ok=True)
    for split in ["train", "val", "test"]:
        s3_path = f"s3://{s3_bucket}/{s3_prefix}/{split}.pt"
        local_path = os.path.join(local_dir, f"{split}.pt")
...
def load_dataset(data_dir):
    """
    Laedt train.pt, val.pt und test.pt aus einem Verzeichnis.

    Parameter
    ---------
    data_dir : str
        Verzeichnis mit den .pt-Dateien.
...
def compute_normalization_stats(data_list):
    """
    Berechnet z-Score Normalisierungsstatistiken auf den Trainingsdaten.

    Parameter
    ---------
    data_list : list[Data]
        Liste von PyTorch Geometric Data-Objekten.

    Rueckgabe
    ---------
    dict mit x_mean, x_std, y_mean, y_std (alle als Tensoren).
    """
    all_x = torch.cat([d.x for d in data_list], dim=0)
    all_y = torch.cat([d.y for d in data_list], dim=0)

    x_mean = all_x.mean(dim=0)
    x_std = all_x.std(dim=0).clamp(min=1e-8)
    y_mean = all_y.mean(dim=0)
    y_std = all_y.std(dim=0).clamp(min=1e-8)
...
def normalize_dataset(data_list, stats):
    """
    Normalisiert einen Datensatz mit vorberechneten Statistiken.

    Parameter
    ---------
    data_list : list[Data]
        Zu normalisierender Datensatz.
    stats : dict
        Normalisierungsstatistiken von compute_normalization_stats().

    Rueckgabe
    ---------
    list[Data] : Normalisierter Datensatz.
    """
    x_mean = stats["x_mean"]
    x_std = stats["x_std"]
    y_mean = stats["y_mean"]
...
class MLP(nn.Module):
    """
    Zweischichtiges MLP mit ReLU-Aktivierung und LayerNorm.

    Wird als Encoder und Decoder im Encode-Process-Decode-Schema
    verwendet.

    Referenz: Pfaff et al. (2020) — MeshGraphNets
    """
...
class GCNProcessor(nn.Module):
    ...
    Optional: Gradient Checkpointing pro Schicht, um VRAM-Bedarf
    von O(L * N * H) auf O(N * H) zu reduzieren.
    ...
    def __init__(self, hidden_dim, num_layers, dropout=0.0,
        ...
        use_gradient_checkpointing : bool
            Aktivierungen nicht speichern, sondern beim Backward-Pass
            neu berechnen. Reduziert VRAM auf Kosten von Rechenzeit.
        """
        super().__init__()
...
def train_one_epoch(model, loader, optimizer, device,
    ...
    use_mixed_precision : bool
        BF16 Mixed Precision verwenden (nur auf CUDA).
...
def main():
    ...
    parser.add_argument(
        ...
        help="Gradient Checkpointing aktivieren (spart VRAM, "
             "kostet Rechenzeit)")
    parser.add_argument(
        "--mixed-precision", action="store_true",
        help="Mixed Precision Training mit BF16 aktivieren "
             "(spart VRAM, nur auf CUDA)")
    ...
    parser.add_argument(
        ...
        help="Ausgabeverzeichnis (Standard: ./output_gcn_efficient)")
    ...
    if resume_checkpoint is not None:
        ...
        log_and_print(logger,
                      ...
                      f"ohne Verbesserung: {epochs_without_improvement}).")
        log_and_print(logger, "")
    ...
    summary_lines = [
        ...
        "=" * 60,
        "",
        f"   Architektur:       GCN Encode-Process-Decode (efficient)",
        f"   Parameter:         {n_params:,}",
        f"   Hidden Dim:        {args.hidden_dim}",
        f"   MP-Runden:         {args.num_layers}",
        f"   Grad. Checkp.:     "
