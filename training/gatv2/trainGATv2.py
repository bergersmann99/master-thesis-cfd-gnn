...
Der einzige Unterschied zu trainGCN.py liegt im Processor:
GATv2Conv (Brody et al., 2022) ersetzt GCNConv (Kipf und Welling, 2017).
Alle anderen Komponenten (Encoder, Decoder, Normalisierung, Loss,
Optimizer, Evaluation) sind bewusst identisch gehalten, um einen
fairen, kontrollierten Architekturvergleich zu gewaehrleisten.

GATv2 vs. GCN:
    - GCN:   Isotrope Aggregation (alle Nachbarn gleich gewichtet)
    - GATv2: Anisotrope Aggregation (dynamische Attention-Gewichte)
    GATv2 behebt die statische Attention-Rangfolge des originalen
    GAT (Velickovic et al., 2018) durch dynamische Attention
    (Brody et al., 2022, Theorem 1).

Multi-Head Attention:
    Jede GATv2-Schicht verwendet mehrere Attention-Koepfe
    (Standard: 4). Die Ausgabe wird konkateniert, wobei die
    Dimensionen pro Kopf so gewaehlt sind, dass die Gesamtdimension
    hidden_dim ergibt (hidden_dim // heads pro Kopf).
...
Verwendung:
    python trainGATv2.py --data-dir ./graph_dataset
    python trainGATv2.py --s3-download --hidden-dim 128 --num-layers 10
    python trainGATv2.py --s3-download --heads 4

Siehe QUELLEN_trainGATv2.md fuer vollstaendige Quellenangaben.
"""
...
def setup_file_logger(output_dir):
    ...
    logger = logging.getLogger("trainGATv2")
...
class GATv2Processor(nn.Module):
    ...
    Im Unterschied zum originalen GAT (Velickovic et al., 2018)
    wird die Nichtlinearitaet (LeakyReLU) NACH der Konkatenation
    angewendet, was vollstaendig dynamische Attention ermoeglicht
    (statische Rangfolge wird behoben, Theorem 1 in Brody et al.).
...
class GATv2Surrogate(nn.Module):
    ...
    Der Encoder und Decoder sind identisch zu GCNSurrogate.
    Nur der Processor unterscheidet sich (GATv2Conv statt GCNConv).
    Dies gewaehrleistet einen fairen Architekturvergleich.
...
def save_checkpoint(model, optimizer, scheduler, epoch, val_loss,
    ...
    checkpoint = {
        ...
        "architecture": "GATv2_Encode-Process-Decode",
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./output_gatv2",
        help="Ausgabeverzeichnis (Standard: ./output_gatv2)")
    ...
    header_lines = [
        "",
        "=" * 60,
        "   GATv2 SURROGAT-MODELL TRAINING",
